...
export const max = operationColumn((array) => {
...
export const min = operationColumn((array) => {
...
export const sum_r = operationRow((b: Value) => {
    if (isValueArray(b)) {
...
export const mean_r = operationRow((b: Value) => {
    if (isValueArray(b)) {
        return sum_r(b) / b.length
    }
    if (isValueSingle(b)) {
        return b
    }
...
// export const simple_moving_average = (n: number) => operationRow((b: Value) => {
//     if (isValueArray(b)) {
//         let size = 0
//         let sma = 0
//         const arr:number[]=[]
