...
import { sma, ema, wma, max, min, moving_normalize, add, div, mul, wilder_moving_average, sub } from '../base.mjs'
...
export function dmi(val: [ValueArray, ValueArray, ValueArray], n: number, w: number, adxMethod: 'ORIGINAL' | 'SMA') {
    ...
    const ATR = atr([close, high, low], n * 2)

    const smaATR = sma([ATR], n * 2)
    const weight = [...batch(add, [
        batch(mul, [
            batch(sub, [
                batch(div, [ATR, smaATR]),
                looseValue(1)
            ]),
            looseValue(1.7)
        ]),
    ...
    // const weight = batch(
    //     add, [
    //     looseValue(weightBase + (1 - weightBase)),
    //     batch(mul, [
    //         looseValue(1 - weightBase),
    //         batch(sub,
    //             [
    //                 looseValue(1),
    //                 moving_normalize([ATR], n)
    //             ]
    //         )
    //     ])
    // ])
    ...
    iterator(length(close), (i) => {
        ...
        // const p = list[i - 1]
        TR_[i] = Math.max(Math.abs(high[i] - low[i]), Math.abs(high[i] - close[i - 1]), Math.abs(low[i] - close[i - 1]))
        const uD = high[i] - high[i - 1]
        const lD = -(low[i] - low[i - 1])
        uDM_[i] = (uD > 0 && uD > lD) ? uD : 0
        lDM_[i] = (lD > 0 && lD > uD) ? lD : 0
    })
    const TR = batch(mul, [TR_, weight])
    const uDM = batch(mul, [uDM_, weight])
    const lDM = batch(mul, [lDM_, weight])
